...
<?php
    ...
    $messageToControl = Swift_message::newInstance();
    $message = Swift_Message::newInstance();
    $message->setSubject('קפיץ קפוץ-הרשמה');
    $messageToControl->setSubject('קפיץ קפוץ-הרשמה');
    $message->setFrom('signup@kkg.co.il');
    $messageToControl->setFrom('noamf98@gmail.com');
    if ($_POST['dadMail']==""){
        $message->setTo($_POST['momMail']);
        }
        elseif ($_POST['momMail']=="") {
            $message->setTo($_POST['dadMail']);
        }
        else{
            $message->setTo($_POST['dadMail']);
            $message->setCc($_POST['momMail']);
        }
    $messageToControl->setTo('signup@kkg.co.il');
    if ($_FILES['perant']['name']!=""){
        ...
        $message->attach($att); 
        $messageToControl->attach($att);
    }
    else{
        $att=Swift_Attachment::fromPath("Harshma_Multi_years.pdf")
            ->setFilename("אישור הורים.pdf");
        $message->attach($att);
    }
    if ($_FILES['medic']['name']!=""){
        ...
        $att=Swift_Attachment::fromPath("kfitz_kfotz_doctor_note.pdf")
            ->setFilename("אישור רפואי.pdf");
        $message->attach($att);
    }
    if ($_FILES['bank']['name']!=""){
        ...
        if (!($site){
            $att = Swift_Attachment::fromPath('/index.files/Horat_Keva.pdf')
                ->setFilename("הוראת קבע.pdf");
            $message->attach($att);
        }
    }
    ...
    $messageToControl->attach($attachment);
    $mailer->send($message);
    $mailer->send($messageToControl);
    if ($site){
