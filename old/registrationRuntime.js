...
var isOld = false,Dir = "../Files/";
var isEdit = false;

function checkPayBok()
{
    if(!isEdit)
    {
        isEdit = true;
        document.getElementById("pricecheckbox").value=="";
        for (var i=0; i<=sessionStorage.clickcount; i++)
        {
            deleteAgeGroup(i);
        }
    }
    else
    {
        ...
        document.getElementById("pricecheckbox").value=="true";
    }
    setPrice();
}
...
function GetYear(){
    ...
    text = "";
    text += "<h1 class=\" w3-right w3-text-right w3-text-white\">";
    text += "בעבור שנה:";
    text += "<select id =\"years\" style=\"text-align:right\"  class=\"w3-select  w3-right w3-text-right\" name=\"year\" onchange = \"changeYear()\">";
    for (i=1; i<arr.length; i++)
    {
        text += "<option style=\"text-align:right\"  value=" + i + " style=\"text-align:right\" selected>";
        text += "<span>" + arr[i][0].replace("\"","").replace("\"","") + "</span>";
        text += "</option>"        
    }
    text += "<option style=\"text-align:right\"  value=" + arr.length + " style=\"text-align:right\">";
    text += "<span>שנים עברו</span></option>"
    text += "</select></h1>";
...
function changeYear(){
    var arr, i = document.getElementById("years").value;
    arr = getCSV("../years.csv");
    if (i == arr.length)
    {
        isOld = true;
        document.getElementById("price").value="";
        document.getElementById("price").placeholder="בחר סכום";
        for (var a = 0; a <= sessionStorage.clickcount; a++)
        {
            deleteAgeGroup(a);
        }
    }
...
function deleteAgeGroup(num)
{
    document.getElementById('start' + num).innerHTML = "";
}
// change out type
...
function setAction(i) {
    ...
    form.action = "a.php?num=" + i + "&price=" + sessionStorage.price + "&numOfPay=" + form.elements['numOfPay'].value + "&out=" + document.getElementById('out').value;
}
function chPrice()
{
    sessionStorage.price = document.getElementById("priceAmount").value;
    document.getElementById('numOfPrice').innerHTML = changePrice(sessionStorage.price);
...
function changePrice(price) {
    ...
    text += "<select style=\"text-align:right\"  class=\" w3-right w3-text-right w3-select\" name=\"numOfPay\" onchange=\"chNumOfPay(this.value)\"><option style=\"text-align:right\" value=\"\">בחר</option><option style=\"text-align:right\"  value=\"1\">כרטיס אשראי: 1 תשלומים</option>";
    price = price / 300;
...
function setPrice() {
    ...
    for (i = 0; i < arr_girls.length; i++) {
        price += calcPriceGirls(arr_girls[i][0], i);
    }
    ...
    var text = "";
    text +="<label class=\"w3-text-white w3-label w3-right w3-text-right\">תשלום כולל</label><input name=\"sumPrice\" class=\"w3-input w3-right w3-text-right\" type=\"text\" value=" + Math.round(price) + " style=\"text-align:middle\"";
    if(!isEdit)
    {
        text+="readonly"
    }
    text += " id = \"priceAmount\" onchange = \"chPrice()\">";
    document.getElementById('price').innerHTML = text;
    document.getElementById('numOfPrice').innerHTML = changePrice(Math.round(price));
    if (isOld|| isEdit)
    {
        document.getElementById("payspecialdiv").innerHTML = "<div class = \"w3-row-padding w3-padding w3-indigo w30border-right w3-border-left w3-border-bottom w3-border-red\">\
                                                                <div class = \"w3-row\">\
                                                                    <label class = \" w3-right w3-text-right\"><span class = \"w3-text-red\">*</span>סיבת תשלום מיוחד</label>\
                                                                </div>\
                                                                <div>\
                                                                    <input name = \"payspecialInfo\" placeholder=\"סיבת תשלום מיוחד\" required></input>\
                                                                </div>\
                                                            </div>";
    }
    else
    {
         document.getElementById("payspecialdiv").innerHTML = "";
    }
    chNumOfPay(-1);
}
// int an array
...
function calcPriceBoysDis(id, num) {
    ...
    {
        amountOfLoss = form.elements['start' + id].value;
    }
    switch (num) {
        case 0:
            price += arr[0][0];
            price -= arr[0][2] * amountOfLoss;
            price *= 0.95;
            price += arr[0][1];
            break;
        case 1:
            price += arr[1][0];
            price -= arr[1][2] * amountOfLoss;
            price *= 0.90;
            price += arr[1][1];
            break;
        case 2:
            price += arr[2][1];
            price -= arr[2][2] * amountOfLoss;
            price *= 0.80;
            price += arr[2][0];
            break;
        default:
            price += arr[3][1];
            price -= arr[3][2] * amountOfLoss;
            price *= 0.80;
            price += arr[3][0];
            break;
    }
    ...
    switch (num) {
        case 0:
            ...
            switch (form.elements['group' + id].value) {
                case '0':
                    price += arr[0][2];
                    price -= (arr[0][4] + arr[0][5]) * amountOfLoss;
                    break;
                case '1':
                    price -= arr[0][4] * amountOfLoss;
                    break;
                case '2':
                    price += arr[0][3];
                    price -= (arr[0][4] + arr[0][6]) * amountOfLoss;
                    break;
                case '3':
                    price -= arr[0][7] * amountOfLoss;
                    break;
            }
            break;
        case 1:
            ...
            switch (form.elements['group' + id].value) {
                case '0':
                    price += arr[1][2];
                    price -= (arr[1][4] + arr[2][5]) * amountOfLoss;
                    break;
                case '1':
                    price -= arr[1][4] * amountOfLoss;
                    break;
                case '2':
                    price += arr[1][3];
                    price -= (arr[1][4] + arr[2][6]) * amountOfLoss;
                    break;
                case '3':
                    price -= arr[1][7] * amountOfLoss;
                    break;
            }
            break;
        case 2:
            ...
            switch (form.elements['group' + id].value) {
                case '0':
                    price += arr[2][2];
                    price -= (arr[2][4] + arr[2][5]) * amountOfLoss;
                    break;
                case '1':
                    price -= arr[2][4] * amountOfLoss;
                    break;
                case '2':
                    price += arr[2][3];
                    price -= (arr[2][4] + arr[0][6]) * amountOfLoss;
                    break;
                case '3':
                    price -= arr[2][7] * amountOfLoss;
                    break;
            }
            break;
        default:
            ...
            switch (form.elements['group' + id].value) {
                case '0':
                    price += arr[3][2];
                    price -= (arr[3][4] + arr[3][5]) * amountOfLoss;
                    break;
                case '1':
                    price -= arr[3][4] * amountOfLoss;
                    break;
                case '2':
                    price += arr[3][3];
                    price -= (arr[3][4] + arr[3][6]) * amountOfLoss;
                    break;
                case '3':
                    price -= arr[3][7] * amountOfLoss;
                    break;
            }
            break;
    }
...
function setAgeGroup(num) {
    var places, arr;
    if (form.elements['place' + num].value == 1 && (form.elements['group' + num].value == 3||form.elements['group'+num].value==2)){
